...
> **옵션 모듈**: 이 모듈은 선택 사항입니다. 건너뛰고 바로 [10 — 정리](10-cleanup.md)로 이동해도 됩니다.
...
복원 후에는 내부 LB annotation이 제거되고 `spec.addresses`에 정적 공인 IP가 다시 설정됩니다. 06 모듈 2절에서 추가한 `parametersRef`는 그대로 유지됩니다.

👁️ **예시** — 외부 Gateway 복원 후 Gateway API 전체 YAML
```yaml
...
spec:
  ...
  infrastructure:
    parametersRef:                    # 06 모듈 2절의 커스터마이징은 유지
      group: ""
      kind: ConfigMap
      name: gw-options
    # 내부 LB annotations는 복원 명령으로 제거됨
  listeners:
...
> 위 YAML은 복원된 구조를 확인하기 위한 예시이며 다시 적용하지 않습니다.
...
마이그레이션 완료 — ingress-nginx는 사라졌고, AFD는 Gateway API 데이터 플레인만 바라봅니다. AFD 리소스(`afd-mig-$SUFFIX`)는 `$RESOURCE_GROUP`에 있으므로 [10 — 정리](10-cleanup.md)의 RG 삭제로 함께 제거됩니다.
...
[← 06 — Gateway 인프라 커스터마이징 (옵션)](06-gateway-customizations.md) 또는 [← 05 — TLS Gateway와 DNS A 레코드](05-tls-gateway-externaldns.md) | 다음: [09 — Istio Gateway API 쿠키 일관 해시·응답 헤더·본문 검증 (옵션)](09-istio-cookie-affinity.md) 또는 [10 — 정리](10-cleanup.md)
